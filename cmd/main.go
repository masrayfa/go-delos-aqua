...
func main() {
	...
	// user router
	mainRouter.appRouter.Handler("GET", "/api/v1/user/*path", helper.StoreOriginalPath(http.StripPrefix("/api/v1/user", stats.Middleware(userRouter))))
	mainRouter.appRouter.Handler("POST", "/api/v1/user/*path", helper.StoreOriginalPath(http.StripPrefix("/api/v1/user", stats.Middleware(userRouter))))
	mainRouter.appRouter.Handler("PUT", "/api/v1/user/*path", helper.StoreOriginalPath(http.StripPrefix("/api/v1/user", stats.Middleware(userRouter))))
	mainRouter.appRouter.Handler("DELETE", "/api/v1/user/*path", helper.StoreOriginalPath(http.StripPrefix("/api/v1/user", stats.Middleware(userRouter))))
